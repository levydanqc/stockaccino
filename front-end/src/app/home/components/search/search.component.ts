...
export class SearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.searchedStock) {
      ...
      this._yahooService.getSearchedStock(stockSymbol).subscribe((data: any) => {
        this.quote = data;
      ...
      this._userService.getUserById(this._cookieService.get('id')).subscribe((data: any) => {
        this.user = data;
  ...
  watch() {
    if (this.searchedStock) this._userService.watchStock(this._cookieService.get('id'), this.searchedStock);
    this.isWatched = true;
  }

  unwatch() {
    if (this.searchedStock) this._userService.unwatchStock(this._cookieService.get('id'), this.searchedStock);
