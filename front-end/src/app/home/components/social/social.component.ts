...
import { Friend } from 'src/app/classes/users/friend';
...
export class SocialComponent implements OnInit {
  ...
  accept(email: string, prenom: string, nom: string) {
    this.requetes = this.requetes?.filter((requete) => {
      return requete.Email !== email;
    });
    this.requetesString = this.requetesString?.filter((requete) => {
      return requete !== email;
    });
  ...
  refuse(email: string) {
    ...
    this.toastr.success(
      "Ce compte a été supprimé de vos requêtes d'amitié.",
      'Succès'
    );
  }
      ...
      if (this.form.valid) {
        if (this.emailList.includes(this.form.get('email')?.value)) {
          if (this.form.get('email')?.value === this.userEmail) {
            this.toastr.error(
              `Vous ne pouvez pas vous envoyer une requête.`,
              'Erreur'
            );
          } else {
            if (
              this.amisString?.includes(this.form.get('email')?.value) ||
              this.requetesString?.includes(this.form.get('email')?.value)
            ) {
              this.toastr.error(
                'Ce compte est déjà dans vos listes.',
                'Erreur'
              );
            } else {
  ...
  delete(email: string) {
    this.amis = this.amis?.filter((ami) => {
      return ami.Email !== email;
    });
    this.amisString = this.amisString?.filter((ami) => {
      return ami !== email;
    });
    ...
    this._userService.getUserById().subscribe((data: any) => {
      ...
      if (this.requetesString && this.requetesString.length > 0) {
        ...
        this.requetesString.forEach((requete) => {
          this._userService.getUserByEmail(requete).subscribe((data: any) => {
            let friend: Friend = {
              Email: requete,
              Prenom: data.Prenom,
              Nom: data.Nom,
